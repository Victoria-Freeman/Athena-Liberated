...
class GetApplication @Inject constructor(
    private val packageRepository: ApplicationRepository
) {
    suspend fun execute(applicationID: String): Result<Application, Error> {
        return try {
            val application = packageRepository.getApplicationByID(packageId = applicationID)
            application?.let {
                Result.Success(application)
            } ?: run {
                Result.Failure(Error.ServerError("Application $applicationID was not found"))
            }
        } catch (e: Exception) {
